	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().GetAccount(mock.Anything, account2.ID).Times(1).Return(account2, nil)
				...
				store.EXPECT().TransferTx(mock.Anything, arg).Times(1).Return(db.TransferTxResult{}, nil)
			},
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(mock.Anything, mock.Anything).Unset()
				store.EXPECT().TransferTx(mock.Anything, mock.Anything).Unset()
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusUnauthorized, recorder.Code)
		...
		},
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationTypeBearer, user3.Username, user3.Username, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(mock.Anything, account3.ID).Times(1).Return(account3, nil)
				store.EXPECT().GetAccount(mock.Anything, account2.ID).Unset()
				store.EXPECT().TransferTx(mock.Anything, mock.Anything).Unset()
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(mock.Anything, mock.Anything).Times(1).Return(db.Account{}, sql.ErrConnDone)
				store.EXPECT().TransferTx(mock.Anything, mock.Anything).Unset()
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(mock.Anything, account1.ID).Times(1).Return(account1, nil)
				store.EXPECT().GetAccount(mock.Anything, account2.ID).Times(1).Return(account2, nil)
				store.EXPECT().TransferTx(mock.Anything, mock.Anything).Times(1).Return(db.TransferTxResult{}, sql.ErrTxDone)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusInternalServerError, recorder.Code)
			},
